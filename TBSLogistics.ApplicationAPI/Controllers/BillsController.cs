    ...
    public class BillsController : ControllerBase
    {
        ...
        public async Task<IActionResult> ExportExcelBill([FromQuery] PaginationFilter filter)
        {
            ...
            using (var package = new ExcelPackage(stream))
            {
                ...
                workSheet.Cells[1, 3].Value = "Loại Hàng Hóa";
                workSheet.Cells[1, 4].Value = "Loại Phương Tiện";
                workSheet.Cells[1, 5].Value = "Phương Thức Vận Chuyển";
                workSheet.Cells[1, 6].Value = "Khách Hàng";
                workSheet.Cells[1, 7].Value = "Đơn Vị Vận Tải";
                workSheet.Cells[1, 8].Value = "Đơn Giá Khách Hàng";
                workSheet.Cells[1, 9].Value = "Đơn Giá Nhà Cung Cấp";
                workSheet.Cells[1, 10].Value = "Doanh Thu";
                workSheet.Cells[1, 11].Value = "Lợi Nhuận";
                workSheet.Cells[1, 12].Value = "Phụ Phí Hợp Đồng";
                workSheet.Cells[1, 13].Value = "Phụ Phí Phát Sinh";
            
                int row = 2;
                foreach (var item in data.dataResponse)
                {
                    workSheet.Cells[row, 1].Value = item.MaVanDon;
                    workSheet.Cells[row, 2].Value = item.LoaiVanDon == "nhap" ? "Nhập" : "Xuất";
                    workSheet.Cells[row, 3].Value = item.LoaiHangHoa;
                    workSheet.Cells[row, 4].Value = item.LoaiPhuongTien;
                    workSheet.Cells[row, 5].Value = item.MaPTVC;
                    workSheet.Cells[row, 6].Value = item.MaKh;
                    workSheet.Cells[row, 7].Value = item.DonViVanTai;
                    workSheet.Cells[row, 8].Value = item.DonGiaKH;
                    workSheet.Cells[row, 9].Value = item.DonGiaNCC;
                    workSheet.Cells[row, 10].Value = item.DoanhThu;
                    workSheet.Cells[row, 11].Value = item.LoiNhuan;
                    workSheet.Cells[row, 12].Value = item.ChiPhiHopDong;
                    workSheet.Cells[row, 13].Value = item.ChiPhiPhatSinh;
                    row++;
                }
