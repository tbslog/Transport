...
import ExcelAddNewRoad from "../../ExcelFile/RoadModule/AddNewRoad.xlsx";
...
const RoadPage = () => {
  ...
  const fetchData = async (page, KeyWord = "") => {
    ...
    const dataCus = await getData(
      `http://localhost:8088/api/Road/GetListRoad?PageNumber=${page}&PageSize=${perPage}&KeyWord=${KeyWord}`
    );
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    ...
    const dataCus = await getData(
      `http://localhost:8088/api/Road/GetListRoad?PageNumber=${page}&PageSize=${newPerPage}&KeyWord=${keySearch}`
    );
  ...
  const handleExcelImportClick = (e) => {
    ...
    const importExcelCus = postFile(
      "http://localhost:8088/api/Road/ReadFileExcel",
      { formFile: file }
    );
    setLoading(false);
  };
